...
TickType_t freertos_rs_xTaskGetTickCountFromISR() {
    return xTaskGetTickCountFromISR();
}

#if (configUSE_TRACE_FACILITY == 1)
UBaseType_t freertos_rs_get_system_state(TaskStatus_t *const pxTaskStatusArray, const UBaseType_t uxArraySize,
...
UBaseType_t freertos_rs_spawn_task(TaskFunction_t entry_point, void *pvParameters, const char *const name,
                                   uint8_t name_len, uint16_t stack_size, UBaseType_t priority,
...
BaseType_t freertos_rs_task_notify_wait(uint32_t ulBitsToClearOnEntry, uint32_t ulBitsToClearOnExit,
...
BaseType_t freertos_rs_task_notify_isr(void *task, uint32_t value, uint8_t action,
...
EventBits_t freertos_rs_event_group_wait_bits(const EventGroupHandle_t event_group, const EventBits_t bits_to_wait_for,
                                              const BaseType_t clear_on_exit, const BaseType_t wait_for_all_bits,
                                              TickType_t ticks_to_wait) {
    return xEventGroupWaitBits(event_group, bits_to_wait_for, clear_on_exit, wait_for_all_bits, ticks_to_wait);
}

EventBits_t freertos_rs_event_group_sync(EventGroupHandle_t event_group, const EventBits_t bits_to_set,
